    ...
    if uploaded_file is not None:
        try:
            ...
            with col2:
                ...
                total_qty = df_processed['Qty'].fillna(0).sum() if 'Qty' in df_processed.columns else 0
                st.metric("Total Quantity", f"{int(total_qty):,}")
            with col3:
                # Calculate total amount
                total_amount = df_processed['Amount'].fillna(0).sum() if 'Amount' in df_processed.columns else 0
