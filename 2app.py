...
def generate_proforma_invoice(df, pi_number=None, po_reference=None, shipment_date=None):
    ...
    # **MODIFICATION START: Aggregate data by StyleID**
    # This ensures that the number of rows is based on unique style codes [6].
    # For descriptive columns, take the first value. For numeric, sum them.
    aggregated_df = df.groupby('StyleID').agg(
        **{'Item Description': ('Item Description', 'first'),
           'Fabric Type': ('Fabric Type', 'first'),
           'HS Code': ('HS Code', 'first'),
           'Composition': ('Composition', 'first'),
           'Country of Origin': ('Country of Origin', 'first'),
           'Qty': ('Qty', 'sum'),
           'Unit Price': ('Unit Price', 'mean'), # Taking mean for Unit Price for the aggregated row
           'Amount': ('Amount', 'sum')
        }
    ).reset_index()

    for _, row in aggregated_df.iterrows(): # Iterate through the aggregated DataFrame
        # [7]
        qty = row.get("Qty", 0) or 0
        price = row.get("Unit Price", 0) or 0
        amount = row.get("Amount", 0) or 0

        # Handle NaN values (though aggregation should have handled most)
    ...
    # **MODIFICATION END: Aggregate data by StyleID**
...
def main():
    ...
    with st.sidebar:
        ...
        **Required Excel Columns:**
        - StyleID (from 'STYLE NO.' in your Excel)
        - Item Description (from 'ITEM DESCRIPTION' in your Excel)
        - Fabric Type (from 'FABRIC TYPE KNITTED / WOVEN' in your Excel)
        - HS Code (from 'H.S NO (8digit)' in your Excel)
        - Composition (from 'COMPOSITION OF MATERIAL' in your Excel)
        - Country of Origin (from 'COUNTRY OF ORIGIN' in your Excel)
        - Qty (from 'QTY' in your Excel)
        - Unit Price (from 'UNIT PRICE FOB' in your Excel)
        - Amount (from 'AMOUNT' in your Excel)
        """)
        st.markdown("---")
    ...
    if uploaded_file is not None:
        try:
            ...
            # **MODIFICATION START: Rename columns and convert types for robust processing**
            # This addresses the issue of unpopulated fields in "Wrong PI.pdf" and "SCRIPT ERROR.pdf" [15-18]
            # due to column name mismatches between input Excel and script expectations [5-7].
            column_mapping = {
                'STYLE NO.': 'StyleID',
                'ITEM DESCRIPTION': 'Item Description',
                'FABRIC TYPE KNITTED / WOVEN': 'Fabric Type',
                'H.S NO (8digit)': 'HS Code',
                'COMPOSITION OF MATERIAL': 'Composition',
                'COUNTRY OF ORIGIN': 'Country of Origin',
                'QTY': 'Qty',
                'UNIT PRICE FOB': 'Unit Price',
                'AMOUNT': 'Amount'
            }

            # Rename columns in the DataFrame based on the mapping
            # Only rename if the column exists in the DataFrame to avoid errors
            df.rename(columns={k: v for k, v in column_mapping.items() if k in df.columns}, inplace=True)

            # Ensure numeric columns are actually numeric, handling potential errors
            for col in ['Qty', 'Unit Price', 'Amount']:
                if col in df.columns:
                    df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
            # **MODIFICATION END: Rename columns and convert types**
            ...
            # [19]
            with col1:
                st.metric("Total Items (Raw Rows)", len(df)) # Changed to Raw Rows as aggregation happens in PDF
            with col2:
                if 'Qty' in df.columns:
                    total_qty = df['Qty'].fillna(0).sum()
                    st.metric("Total Quantity (Raw Sum)", f"{total_qty:,.0f}")
            with col3:
                if 'Amount' in df.columns:
                    total_amount = df['Amount'].fillna(0).sum()
                    st.metric("Total Amount (Raw Sum)", f"${total_amount:,.2f}")
            ...
            if st.button("🚀 Generate Proforma Invoice PDF", type="primary", use_container_width=True):
                try:
                    with st.spinner("Generating PDF in Landmark format... Please wait"):
                        pdf_bytes, final_total_qty, final_total_amount = generate_proforma_invoice(
                    ...
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        # After aggregation, the number of items corresponds to unique StyleIDs
                        st.metric("Total Unique Style Items (Invoiced)", df['StyleID'].nunique() if 'StyleID' in df.columns else 0)
                    with col2:
                        st.metric("Total Quantity (Invoiced)", f"{final_total_qty:,.0f}")
                    with col3:
                        st.metric("Total Amount (Invoiced)", f"${final_total_amount:,.2f}")
